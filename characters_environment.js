...
function game(){
  ...
  if(gameConfig.status==='start'){
    ...
    textSize(40);
    textAlign(CENTER);
    text("Click Play button to start! ", gameConfig.screenX/2, gameConfig.screenY/2);
    textSize(40);
  ...
  if(gameConfig.status==='gameover'){
    ...
    textSize(15);
    text("Press SpaceBar to Restart", gameConfig.screenX/2, gameConfig.screenY/2+135);
    textSize(40);
...
function changeGameStatud(character){
  if(RU!=null && gameConfig.status==="start") {
    world_start.play();
    initializeCharacterStatus(mario);
    gameConfig.status= "play";
    document.getElementById("SD").innerHTML="Game Started!";
  }
...
function manualControl(character){
  ...
  if(character.live){
    console.log(RU[0].pose.rightWrist.x);
    console.log(RU[0].pose.rightWrist.y);
    if(RU[0].pose.rightWrist.x<250){
      character.velocity.x+=gameConfig.moveSpeed;
      character.changeAnimation('move');
      character.mirrorX(1);
    }

    if(RU[0].pose.rightWrist.x>350){
      character.velocity.x-=gameConfig.moveSpeed;
      character.changeAnimation('move');
      character.mirrorX(-1);
    }

    if(RU[0].pose.rightWrist.x>250 && RU[0].pose.rightWrist.x<350){ 
...
function jumping(character){
	if( (RU[0].pose.rightWrist.y<300 &&character.live) || (touchIsDown&&character.live) ){
...
function checkStatus(character){    
  ...
  if(character.live==false && character.liveNumber==0){
    ...
    GO.play();
    document.getElementById("SD").innerHTML="Waiting for restart!";
  }
